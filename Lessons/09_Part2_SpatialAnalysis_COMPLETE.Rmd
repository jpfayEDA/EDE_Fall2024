...
ggplot(data=huc8_sf,aes(fill = ACRES)) + geom_sf()
...
ggplot() + 
  ...
  geom_sf(data=upper_neuse_sf,fill='yellow',color='blue')

mapview(huc8_sf,col.regions='grey') +
  mapview(upper_neuse_sf, col.regions='yellow',color='blue')
```
...
wind_data <- read.csv(
  ...
  mutate(
    HUC_8 = substr(HUC_12,0,8)
  ) %>% 
  group_by(HUC_8) %>% 
  summarize(
    WindEnergy1 = sum(kWhkm2day),
    WindEnergy2 = sum(AvgWindEnergy)
  )
  
#Join to HUC_8 features
huc8_sf.join <- huc8_sf %>% 
  left_join(wind_data, by='HUC_8')

#View the outputs
ggplot(data=huc8_sf.join) +
  geom_sf(aes(fill=WindEnergy2))
...
huc8_sf_utm %>% st_filter(triCo) %>% mapview() + mapview(triCo)


#Intersect the HUC_8s
huc8_sf_utm %>% st_intersection(triCo) %>% mapview() #+ mapview(triCo)
