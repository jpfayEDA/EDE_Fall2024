...
mapview(huc8_sf,col.regions='grey') + 
  mapview(upper_neuse_sf,col.regions='yellow',color='red')
...
#View the outputs
ggplot() + geom_sf(data=huc8_wind,aes(fill=Wind1))
mapview(huc8_wind,zcol='Wind1')
```
...
#Map the data
ggplot() + geom_sf(huc2_sf,aes(fill=ACRES))
...
huc8_sf_utm %>% 
  st_filter(triCo) %>% 
  mapview() +
  mapview(triCo,color='red',alpha.regions = 0)
...
#Create the buffer
pt_buffer = st_buffer(theSite_sfc_transformed,30000)
